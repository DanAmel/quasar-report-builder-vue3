...
export default {
  ...
  methods: {
    ...
    /**
     * Cette Fonction convertir la donnée en un objet exploitable par le générateur de rapport
     *
     * reportPdf: Le parametrage du rapport telque sauvegarder en base de données
     * */
    ...
    async getStatsData(type = "PDF") {
      ...
      let customFilter = cloneDeep(this.filter)
      let start_date = this.datesRange.from ? moment(this.datesRange.from).format( 'YYYY-MM-DD') : moment(this.datesRange).format( 'YYYY-MM-DD')
      let end_date = this.datesRange.to ?  moment(this.datesRange.to).format( 'YYYY-MM-DD') : moment(this.datesRange).format('YYYY-MM-DD')

      if(start_date !== end_date &&  get(customFilter, 'operand_date.code', "between") !== "between"){
      ...
      customFilter.conditions.forEach(x =>{
      ...
      let params = null
      if(get(customFilter, 'request_mode') === "sql"){
        customFilter.request_editor.params = this.params
        params = this.params
      }
      else
