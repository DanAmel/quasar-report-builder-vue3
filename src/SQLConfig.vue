<template>
  <q-dialog ref="dialogSQLRef" @hide="onDialogHide" :persistent="true">
                  ...
                  <div class="row">
                    <div class="col-3">
                      <q-input v-model="item.code" class="q-ml-md"  dense label="colonne" disable
                               @m-blur="v$.columns.$each.$response.$data[index].code.$touch"
                               :error="v$.columns.$each.$response.$data[index].code.$error"
                    ...
                    <div class="col-4">
                      <q-input v-model="item.name" class="q-ml-md"  dense label="Nom" filled
                               @m-blur="v$.columns.$each.$response.$data[index].name.$touch"
                               :error="v$.columns.$each.$response.$data[index].name.$error"
                      />
                    </div>
                    <div class="col-5">
                      ...
                      <q-select  dense outlined class="q-ml-md" clearable
                                 ...
                                 @m-blur="v$.columns.$each.$response.$data[index].type.$touch"
                                 :error="v$.columns.$each.$response.$data[index].type.$error"
                                 error-message="Champ requis.">
...
<!--        </div>-->
      ...
      <!-- buttons example -->
      <q-card-actions class="bg-white text-primary relative-position">
...
export default {
  ...
  methods: {
    ...
    show() {
      this.$refs.dialogSQLRef.show()
    },

    // following method is REQUIRED
    // (don't change its name --> "hide")
    hide() {
      this.$refs.dialogSQLRef.hide()
    },

    showNotification(notification) {
      //this.$refs.dialogRef.$parent.$emit('showNotification', notification)
      console.log("this.$parentthis.$parent", this.$parent)
    ...
    async onVerify(){
      ...
      await promise.then(async (res) =>{
        ...
        console.log("ijjijijiahdihaihaidhadahi")
